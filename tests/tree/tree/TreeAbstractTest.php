...
abstract class TreeAbstractTest extends TestCase
{
    ...
    /**
     * These tests are ordered so that we first establish that we can get nodes into the abstractTree and the
     * abstractTree is valid. So the addNode and setNodes methods are tested first.
     *
     * Once we have established we can get nodes into the abstractTree, then we can rely on those methods to populate
     * the abstractTree for further testing.
     */
    ...
    /**
     * Now that we have established we can get nodes into the abstractTree and the abstractTree is valid, we can work
     * our way through the remaining methods.  These next methods being tested are located in TreeAbstract.
     */
    ...
    public function testVerifyTreeSearchInitialConditionsSetsDefaultsProperly() : void
    {
        ...
        /**
         * verify that the number of nodes returning from the search equals the total number of nodes in the
         * abstractTree to begin with.  So the callback was defaulted properly because it always returns true
         */
    ...
    public function testGetTreeDepthFirstFullTree(): void
    {
         ...
         */
        $actualResult = array_values(array_map(function ($node) {
    ...
    /**
     * tests that the search returns partial abstractTree in the correct order when starting from interior node with
     * nodeid = 2
     *
    ...
    public function testGetTreeDepthFirstWhereCallbackDoesNotAlwaysReturnTrue(): void
    {
         ...
         * get the nodeid from each node into an array and reindex the array
         */
        $actualResults = array_values(array_map(function ($node) {
            return $node->getNodeId();
        }, $searchResult));
    ...
    public function testGetTreeBreadthFirstThrowsExceptionWithBadMaxLevelsParameter() : void
    {
        $nodeArray = $this->makeNodeArray($this->fixture->makeArrayOfNodeIdsForTree());
        $this->tree->setNodes($nodeArray);
        $this->expectException(BadSearchLevelsException::class);
    ...
    {
        /**
         * need to change the equals expectations on each node so that when the abstractTree calls getChildren with
         * leaf as its argument, the abstractTree knows that leaf is in the abstractTree.
         */
        $nodeArray = $this->makeNodeArray($this->fixture->makeArrayOfNodeIdsForTree());
        $this->tree->setNodes($nodeArray);
